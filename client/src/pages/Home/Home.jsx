...
export default function Home() {
  ...
  const nextVendors = () => {
    setStartIndex((prevIndex) =>
      prevIndex + (window.innerWidth < 768 ? 1 : 3) >= vendors.length
        ? 0
  ...
  const prevVendors = () => {
    setStartIndex((prevIndex) =>
      prevIndex - (window.innerWidth < 768 ? 1 : 3) < 0
        ? vendors.length - (window.innerWidth < 768 ? 1 : 3)
        : prevIndex - (window.innerWidth < 768 ? 1 : 3)
    );
    setPrevClicked(true);
    ...
    <div className={styles.pageContainer}>
      <div className={styles.vendorContainer}>
        ...
        <div className={styles.vendorCarousel}>
          <div
            className={`${styles.navIcon} ${prevClicked ? styles.clicked : ""}`}
            onClick={prevVendors}
          >
            ...
            {vendors
              ...
              .map((vendor, index) => (
                <div
                  ...
                  key={`vendor-${vendor.vendorName}-${index}`}
                >
          ...
          <div
            className={`${styles.navIcon} ${nextClicked ? styles.clicked : ""}`}
            onClick={nextVendors}
          >
        ...
        <div className={styles.categoriesContainer}>
          {categories.map((category, index) => (
            ...
            >
              <div className={styles.categoryTile} key={`category-${index}`}>
                <img
